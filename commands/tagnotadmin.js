...
async function tagNotAdminCommand(sock, chatId, senderId, message) {
    try {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'My lord, pls make me an admin first.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'Only admins can use this command b*tch.' }, { quoted: message });
            return;
        }
