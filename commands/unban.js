...
async function unbanCommand(sock, chatId, message) {
    ...
    if (isGroup) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'Nigga make the bot an admin to use .unban', ...channelInfo }, { quoted: message });
            return;
        }
        if (!isSenderAdmin && !message.key.fromMe) {
            await sock.sendMessage(chatId, { text: 'Only group admins can use .unban b*tch', ...channelInfo }, { quoted: message });
            return;
        }
    } else {
        const senderId = message.key.participant || message.key.remoteJid;
    ...
    if (!userToUnban) {
        await sock.sendMessage(chatId, { 
            text: 'Nigga mention the user or reply to their message to unban!', 
            ...channelInfo 
        }, { quoted: message });
        return;
    }
