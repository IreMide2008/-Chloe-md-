...
async function stickercropCommand(sock, chatId, message) {
    ...
    if (!mediaMessage) {
        await sock.sendMessage(chatId, { 
            text: 'Nigga reply to an image/video/sticker with .crop, or send an image/video/sticker with .crop as the caption.',
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
    ...
    try {
        ...
        if (!mediaBuffer) {
            await sock.sendMessage(chatId, { 
                text: 'Failed to download media. Ish try again.',
                contextInfo: {
                    forwardingScore: 999,
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
