...
async function stickerCommand(sock, chatId, message) {
    ...
    if (!mediaMessage) {
        await sock.sendMessage(chatId, { 
            text: 'Motherfvcker, reply to an image/video with .sticker, or send an image/video with .sticker as the caption.',
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
    ...
    try {
        ...
        const json = {
            ...
            'sticker-pack-name': settings.packname || '©️Chloe™️',
            'emojis': ['🤖']
        };
        ...
        if (isAnimated && finalBuffer.length > 900 * 1024) {
            try {
                ...
                if (fs.existsSync(tempOutput3)) {
                    ...
                    const json2 = {
                        'sticker-pack-id': crypto.randomBytes(32).toString('hex'),
                        'sticker-pack-name': settings.packname || 'LADY KUSHINA',
                        'emojis': ['🤖']
                    };
