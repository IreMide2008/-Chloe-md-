...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'Nigga make the bot an admin first.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'GTF, only group admins can use the kick command.' }, { quoted: message });
            return;
        }
    }
    ...
    if (usersToKick.length === 0) {
        await sock.sendMessage(chatId, { 
            text: 'Mention the user or reply to their message to kick my lord'
        }, { quoted: message });
        return;
    }
