...
async function sudoCommand(sock, chatId, message) {
    ...
    if (!isOwner) {
        await sock.sendMessage(chatId, { text: '❌ Only My luv can add/remove sudo users. Use .sudo list to view.' },{quoted :message});
        return;
    }

    const targetJid = extractMentionedJid(message);
    if (!targetJid) {
        await sock.sendMessage(chatId, { text: 'My lord, pls mention a user or provide a number.' },{quoted :message});
        return;
    }
    ...
    if (sub === 'del' || sub === 'remove') {
        ...
        if (targetJid === ownerJid) {
            await sock.sendMessage(chatId, { text: 'Owner cannot be removed b*tch.' },{quoted :message});
            return;
        }
