...
async function updateCommand(sock, chatId, message, zipOverride) {
    ...
    if (!message.key.fromMe && !isOwner) {
        await sock.sendMessage(chatId, { text: 'Only bot owner or sudo can use .update b*tch' }, { quoted: message });
        return;
    }
    try {
        // Minimal UX
        await sock.sendMessage(chatId, { text: '🔄 Updating bot,  ome moment…' }, { quoted: message });
        if (await hasGitRepo()) {
            // silent
