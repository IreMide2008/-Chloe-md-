...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match.startsWith('set')) {
        ...
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '🔥 Invalid action. Choose: delete, kick, or warn, dattebane!' }, { quoted: message } );
        }
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `🔥 @${senderId.split('@')[0]} bad words are not allowed here, dattebane!`,
                mentions: [senderId]
            });
            break;
