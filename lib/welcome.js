...
async function handleWelcome(sock, chatId, message, match) {
    ...
    if (lowerCommand === 'on') {
        if (await isWelcomeOn(chatId)) {
            return sock.sendMessage(chatId, { text: '🔥 Welcome messages are *already enabled*, dattebane!', quoted: message });
        }
        await addWelcome(chatId, true, 'Welcome {user} to {group}! 🎉 Believe it!');
        return sock.sendMessage(chatId, { text: '✅ Welcome messages *enabled* with simple message. Use *.welcome set [your message]* to customize, dattebane!', quoted: message });
    }
    ...
    if (lowerCommand === 'set') {
        if (!customMessage) {
            return sock.sendMessage(chatId, { text: '🔥 Please provide a custom welcome message. Example: *.welcome set Welcome to the group, dattebane!*', quoted: message });
        }
...
async function handleGoodbye(sock, chatId, message, match) {
    ...
    if (lower === 'on') {
        ...
        await addGoodbye(chatId, true, 'Goodbye {user} 👋 Believe it!');
    ...
    if (lower.startsWith('set ')) {
        ...
        if (!customMessage) {
            return sock.sendMessage(chatId, { text: '🔥 Please provide a custom goodbye message. Example: *.goodbye set Goodbye, dattebane!*', quoted: message });
        }
